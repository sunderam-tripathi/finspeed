...
function Footer({ onNav }) {
  ...
  return (
    <footer className="store-footer" style={{ background:'var(--steel-100)', color:'var(--text-body)', padding:'var(--space-9) var(--space-7) var(--space-6)', borderTop:'1px solid var(--border-subtle)' }}>
      <div className="store-footer-grid" style={{ maxWidth:'var(--container-max)', margin:'0 auto', display:'grid', gridTemplateColumns:'1.6fr 1fr 1fr 1fr', gap:'var(--space-7)' }}>
        <div className="store-footer-brand">
          <div style={{ display:'inline-flex', alignItems:'center', gap:10, marginBottom:'var(--space-4)' }}>
            <img src="/assets/logos/finspeed-mark.png" alt="Finspeed" style={{ height:40 }} />
            <span style={{ font:'var(--fw-bold) 22px/1 var(--font-display)', letterSpacing:'0.06em', textTransform:'uppercase', color:'var(--ink-900)' }}>Finspeed</span>
          </div>
      ...
      <div className="store-footer-bottom" style={{ maxWidth:'var(--container-max)', margin:'var(--space-7) auto 0', paddingTop:'var(--space-5)', borderTop:'1px solid var(--border-subtle)', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:12 }}>
        ...
        <span style={{ font:'var(--fw-regular) var(--fs-2xs)/1 var(--font-mono)', color:'var(--text-muted)' }}>© 2025 Finspeed · MK Electric · Greater Noida</span>
      </div>
    </footer>
  );
}
