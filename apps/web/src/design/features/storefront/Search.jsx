...
function Search({ query, setQuery, onProduct, onNav }) {
  ...
  return (
    <article className="editorial-search-page">
      <header className="editorial-search-hero">
        ...
        <h1>Search the range.</h1>
        <p>Search by model, ride profile, wheel size, or the terrain you have in mind.</p>
      </header>
          ...
          <div className="editorial-search-results">
            ...
            {results.length ? (
                ...
                {results.map((product, index) => (
                  <button type="button" key={product.id} onClick={() => onProduct(product.id)}>
                    ...
                    <span className="editorial-search-list__image">
                      <img src={productImage(product.id, 480)} srcSet={productImageSrcSet(product.id)} sizes="150px" alt="" />
                    </span>
                    ...
                    <span className="editorial-search-list__price">₹{product.price.toLocaleString('en-IN')}</span>
              ...
              <div className="editorial-search-empty">
                ...
                <p>Terms such as mountain, city, hybrid, 29 inch, or geared may reveal the right frame.</p>
                <button type="button" className="editorial-cta editorial-cta--primary" onClick={() => onNav('shop')}>Explore the complete edit <i data-lucide="arrow-right" aria-hidden="true" /></button>
              </div>
            )}
          </div>
        )}
