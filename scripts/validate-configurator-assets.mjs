  ...
  for (const entryRecord of customEntries) {
    ...
    for (const theme of canonical.themes || []) {
      ...
      for (const record of records) {
        const expectedAlphaMode = theme === 'light' ? 'transparent' : 'opaque';
...
async function validateRuntimeResolver(manifest) {
  ...
  for (const sku of catalog.skus || []) {
    ...
    const resolved = runtime.resolveBuild({
      version: catalog.schemaVersion,
      rideType: model.rideType,
      modelId: model.id,
      skuId: sku.id,
      fit: { wheel: sku.wheel, frameSize: null },
    });
    ...
    if (sku.visual.assetKey) {
      check(Boolean(manifestCustom), `runtime custom visual ${sku.visual.assetKey} for ${sku.id} is absent from the manifest`);
      if (manifestCustom) {
...
async function main() {
  ...
  check(manifest.contractVersion === '1.0.0', 'contractVersion must be 1.0.0');
  ...
  for (const product of manifest.products || []) {
    ...
    for (const selectionClass of REQUIRED_SELECTION_CLASSES) {
      ...
      check(Array.isArray(entry?.assets) || Array.isArray(entry?.families), `${label} must declare assets or visual families`);
