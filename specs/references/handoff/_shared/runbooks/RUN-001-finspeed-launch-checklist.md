...
- Run analytics contract suite (Playwright GA4 fixture per slice contract plan) to validate schema + consent gating before deploy.
...
- Validate catalog CSV passes schema check (`catalog-item.schema.json`) and diff results stored.
- Spot-check `/catalog` LCP via Lighthouse CI report stays within budget.
...
- Spot-check GA4 debug view for `site.interaction.analytics.v1` events.
  - Ensure consent decline path suppresses events; capture DebugView screenshot for release notes.
...
- Monitor catalog filter/comparison metrics in GA4 dashboard (filters ≥ 40% interaction, comparison usage trend).
...
- If catalog schema validation fails, revert to previous CSV and notify product team to correct data before redeploy.
